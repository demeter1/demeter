...
#pragma compile(Icon, "demeter\images\icons\icon.ico")
#pragma compile(FileDescription, demeter - Clash of Clans Bot)
#pragma compile(ProductName, demeter - Clash of Clans Bot)
#pragma compile(ProductVersion, 03)
#pragma compile(FileVersion, 03)

#include <GUIConstants.au3>

$sBotVersion = "03"
$sBotTitle = "DEMETER - v." & $sBotVersion

If FileExists(@ScriptDir & "\.developer") Then
	$sBotTitle = "DEMETER - In Development"
EndIf
...
#include "demeter\functions\functions.au3"
...
$ret = DllCall(@ScriptDir & "\demeter\demeter.dll", "str", "DemeterBotRedLineCheck", "ptr", $hHBitmap, "int", 1, "int", 1, "int", 0, "int", 0, "int", 0)
...
If Not IsArray($ret) Then
	...
	EndIf
ElseIf $ret[0] = -2 Then
	MsgBox(48, "www.facebook.com/vdie03", GetLangText("msgLicense") & @CRLF & @CRLF & "UNDER MAINTENANCE LASTEST")
EndIf

If IniRead(@LocalAppDataDir & "\demeter.ini", "default", "1", "") = "" Or IniRead(@LocalAppDataDir & "\demeter.ini", "default", "2", "") = "" Then
	GUICtrlSetImage($btnBBValidate, @ScriptDir & "\images\Resource\bad.bmp")
	GUICtrlSetTip($btnBBValidate, GetLangText("tipBBValidBad"))
Else
	GUICtrlSetData($inpBBPassword, _Decrypt(IniRead(@LocalAppDataDir & "\demeter.ini", "default", "2", "")))
	_btnBBValidate()
